...
<body>
    ...
    <main>
        <section id="supervisor-project">
            <div id="add-project">
                <h1>Proposed Title</h1>
                <form id="add-title-form" action="../POST/add-project.php" method="post">
                    <input type="text" id="title" name="title" placeholder="Add Title Project" required>
                    <select name="project-types" id="project-types">
                        <option value="none" selected disabled>Project Type</option>
                    ...
                    <button type="submit" name="submit" id="add-title-btn">ADD TOPIC</button>
                </form>
            </div>
        ...
        <section id="project-approval">
            ...
            <?php 
                if (count($_POST) > 0) {
                    $connect = OpenCon();
                    $searchTerm = mysqli_real_escape_string($connect, $_POST['search-title']);
                    $sql = "SELECT id, student_id, title, propose, description FROM project WHERE title LIKE '%" . $searchTerm . "%' AND supervisor_approval_status = 'Pending'";
